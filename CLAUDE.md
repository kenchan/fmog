...
All domain logic lives in `lib/fmog/` as Ruby modules, with the exception of `Fetcher` which is a class:
...
- **`db.rb`** — Singleton SQLite connection with XDG Base Directory Specification support (`$XDG_DATA_HOME/fmog/fmog.db`, defaulting to `~/.local/share/fmog/fmog.db`). Idempotent schema migration on connect. Uses `DB.reset!` in tests to swap in `:memory:` databases.
...
- **`fetcher.rb`** — `Fetcher` is a class with `self.fetch(feed_id)` and `self.fetch_all` as the public API, delegating to private instance methods. Handles RSS 2.0 and Atom format differences via `extract_*` helpers.
...
- **All commit messages must be in English.** Use the conventional commits format: `type: description` (e.g., `feat: add pagination to item list`).
- **All pull request titles and descriptions must be in English.**
